...
public class MembersServiceImpl {
    ...
    public Members addMember(int userId, int rolePermissionId, int ownerTypeId, int ownerId, 
        ...
        Members existingMember = getMemberByUserAndOwner(userId, ownerTypeId, ownerId);
        if (existingMember != null) {
            throw new IllegalStateException("User " + userId + " is already a member of this resource");
        }
    ...
    public Members getMemberByUserAndOwner(int userId, int ownerTypeId, int ownerId) {
        try {
            return jdbcTemplate.queryForObject(
    ...
    public boolean isUserAdmin(int userId, int ownerTypeId, int ownerId) {
        // Assuming role ID 1 is for admin (you may need to adjust this based on your role IDs)
        Integer count = jdbcTemplate.queryForObject(
            "SELECT COUNT(*) FROM Members " +
