...
import vn.ypp4.quanphan.service.mapper.ReactionRowMapper;
...
public class ReactionServiceImpl {
    ...
    public int updateReaction(int id, String reactionsName, String shortCode, Integer categoryId, String icon) {
        ...
        if (shortCode != null && !finalShortCode.equalsIgnoreCase(existingReaction.getShortCode())) {
            ...
            if (count != null && count > 0) {
                throw new IllegalStateException("Another reaction with short code '" + finalShortCode + "' already exists");
            }
        }
    ...
    public int deleteReaction(int id) {
        ...
        if (usageCount != null && usageCount > 0) {
            throw new IllegalStateException("Cannot delete reaction as it is being used by " + usageCount + " comment reaction(s)");
        }
