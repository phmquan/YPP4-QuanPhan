...
import vn.ypp4.quanphan.service.mapper.row.RolePermissionRowMapper;
...
public class RolePermissionServiceImpl {
    ...
    private void validatePermission(String permissionName, String permissionCode) {
        ...
        // Ensure permission code is in uppercase with underscores (e.g., "ADMIN",
        // "READ_ONLY")
        if (!permissionCode.matches("^[A-Z][A-Z0-9_]*$")) {
            throw new IllegalArgumentException(
                    "Permission code must be in uppercase and can only contain letters, numbers, and underscores");
        }
    }
}
