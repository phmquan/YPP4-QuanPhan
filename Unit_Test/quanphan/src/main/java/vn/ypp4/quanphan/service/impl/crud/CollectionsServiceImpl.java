...
import vn.ypp4.quanphan.service.mapper.CollectionsRowMapper;
...
public class CollectionsServiceImpl {
    ...
    @Transactional
    public Collections createCollection(String collectionName, LocalDateTime createdAt, int createdBy, int workspaceId) {
        ...
        if (isCollectionNameExists(collectionName, workspaceId)) {
            throw new IllegalStateException("A collection with name '" + collectionName + "' already exists in this workspace");
        }
    ...
    public int updateCollection(int id, String collectionName, LocalDateTime updatedAt, int updatedBy) {
        ...
        // If collection name is being changed, check for duplicates in the same workspace
        if (collectionName != null && !finalCollectionName.equalsIgnoreCase(existingCollection.getCollectionName())) {
            if (isCollectionNameExists(finalCollectionName, existingCollection.getWorkspaceId())) {
                throw new IllegalStateException("Another collection with name '" + finalCollectionName + "' already exists in this workspace");
            }
        }
